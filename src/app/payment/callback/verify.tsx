...
export default function VerifyPayment({
    ...
    if (loading) {
        return (
            <div className="max-w-md mx-auto bg-slate-800 rounded-xl shadow-lg p-8 text-center border border-slate-700">
                ...
                <h1 className="text-xl font-bold text-sky-300">در حال تأیید پرداخت...</h1>
                <p className="text-gray-400 mt-2">لطفاً منتظر بمانید.</p>
            </div>
        );
    }
    ...
    if (error) {
        return (
            <div className="max-w-md mx-auto bg-slate-800 rounded-xl shadow-lg p-8 text-center border border-red-700/50">
                ...
                <Link href="/" className="px-6 py-3 bg-sky-600 hover:bg-sky-700 text-white font-semibold rounded-lg transition-colors inline-block">
                    بازگشت به صفحه اصلی
                </Link>
            </div>
        );
    ...
    if (isOrderUpdateFailed) {
        return (
            <div className="max-w-md mx-auto bg-slate-800 rounded-xl shadow-lg p-8 text-center border border-amber-700/50">
                ...
                <p className="text-gray-300 mb-4">پرداخت شما با موفقیت انجام شد، اما ثبت وضعیت سفارش در سیستم با مشکل مواجه شد.</p>
                ...
                {result.orderId && (
                    <div className="bg-slate-900/50 rounded-lg p-4 mb-6 border border-slate-700">
                        ...
                        <p className="text-xl text-sky-300 nums">#{result.orderId}</p>
                    </div>
                )}
    ...
    if (isAlreadyProcessing) {
        return (
            <div className="max-w-md mx-auto bg-slate-800 rounded-xl shadow-lg p-8 text-center border border-sky-700/50">
                <div className="w-20 h-20 mx-auto mb-6 rounded-full bg-sky-500/20 text-sky-400 flex items-center justify-center">
                    <svg className="animate-spin h-10 w-10" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                        <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                        <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                    </svg>
                </div>
                <h1 className="text-2xl font-bold mb-4 text-sky-300">تراکنش در حال پردازش</h1>
                <p className="text-gray-300 mb-6">پرداخت شما در حال تأیید است. لطفاً چند لحظه صبر کنید.</p>
                {result.orderId && (
                    <div className="bg-slate-900/50 rounded-lg p-4 mb-6 border border-slate-700">
                        <p className="text-sm text-gray-400 mb-1">شماره سفارش</p>
                        <p className="text-xl text-sky-300 nums">#{result.orderId}</p>
    ...
    return (
        <div className="max-w-md mx-auto bg-slate-800 rounded-xl shadow-lg p-8 text-center border border-slate-700">
            ...
            {isSuccess && result.refId && (
                <div className="bg-slate-900/50 rounded-lg p-4 mb-4 border border-slate-700">
                    <p className="text-sm text-gray-400 mb-1">کد پیگیری تراکنش</p>
                    <p className="text-xl text-sky-300 nums">{result.refId}</p>
                </div>
            )}

            {isSuccess && result.cardNumber && (
                <div className="bg-slate-900/50 rounded-lg p-4 mb-4 border border-slate-700">
                    <p className="text-sm text-gray-400 mb-1">شماره کارت پرداخت‌کننده</p>
                    <p className="text-lg text-gray-200 nums" dir="ltr">{result.cardNumber}</p>
                </div>
            )}
