...
export default function SolutionsGrid({ solutions, activeId, onSelect }: SolutionsGridProps) {
  ...
  return (
    <section dir="rtl" className="max-w-6xl mx-auto px-4 pb-16" aria-label="سایر خدمات">
      <h2 className="text-2xl font-bold text-white mb-8 text-center">همه خدمات</h2>
      <motion.div
      ...
      >
        {otherSolutions.map((s) => (
          <motion.button
            ...
            className={`
              group text-right bg-slate-800/30 border border-slate-700/20 rounded-xl p-5
              transition-all duration-300
              hover:-translate-y-1 hover:border-[color:var(--accent-border)] hover:bg-slate-800/50
              hover:shadow-lg focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-sky-400
            `}
            style={{
          ...
          >
            <div className="flex items-center gap-3 mb-3">
              <div
                className={`w-10 h-10 rounded-xl ${s.color.bg} ${s.color.primary} flex items-center justify-center group-hover:scale-110 transition-transform`}
              >
                {s.icon}
              </div>
